...
def reader(data):
    ...
    mask = cv.bitwise_and(image, image, mask = yellow_mask)
    ...
    edges = canny_detector(blur)

    # Limit the Region of Interest
    region = region_selector(edges)
    ...
    if hough is not None:
        ...
        left_gradient = 0.0
        right_gradient = 0.0
        left_line, right_line = lane_lines(result, hough)
